function [det_sam, det_tim, det_amp, det_score, ncc, nlg,file_dbrms] = snap_detect_func(y,kern_mat_file,min_det_score,fs,use_par)
%  
...
eval(['load ' kern_mat_file]);  % load the kern file 
kern=cat(2,zeros(1,100), kern);  % zero pad from the left by 100 points 
pk=pk+100; klen=klen+100;  % adjust peak position and length of kernel 

% bandpass and envelope the waveforms 
pfilt=bandpass_del(y,flt_low,flt_high,fs,5);  % filter 5th order butter 
